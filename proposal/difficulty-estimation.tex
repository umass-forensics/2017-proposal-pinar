...
\para{Ethereum.} Ethereum operates very similarly to Bitcoin. Miners solve a POW problem that is more complicated
...
%Using Bitcoin's algorithm, difficulty for the $i+1$st time is
%\begin{align}
...
Let D be a sequence generated by Equation~\ref{eq:original-bitcoin} where the first element of the sequence is $D_0 = 1$. We can talk about the expected value of a term in sequence D, given its preceding term and the new data we see. 
\begin{align}
...
\item Attackers can easily lie about the timestamps on the blocks with our new estimator. Therefore, an alternative estimator I plan to develop uses status reports, which is a block header except that the POW does not satisfy the current target. Instead, the minimum hash value in the report represents the hash found since the last block broadcast on
...
\item What are the advantages and disadvantages of status reports compared to the time-based estimator? \item What are the Chernoff bounds associated with these estimators?
\end{enumerate}
